//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.4-2 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2015.07.03 at 11:04:58 AM EEST 
//


package tr.com.agem.arya.metadata.xul.impl;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for modetreecellAttributeType.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="modetreecellAttributeType">
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string">
 *     &lt;enumeration value="none"/>
 *     &lt;enumeration value="normal"/>
 *     &lt;enumeration value="undetermined"/>
 *   &lt;/restriction>
 * &lt;/simpleType>
 * </pre>
 * 
 */
@XmlType(name = "modetreecellAttributeType")
@XmlEnum
public enum ModetreecellAttributeType {


    /**
     * none: No progress meter is displayed. The cell text is displayed instead.
     * 
     */
    @XmlEnumValue("none")
    NONE("none"),

    /**
     * normal: The cell uses its value attribute to determine the amount of the bar that is filled in.
     * 
     */
    @XmlEnumValue("normal")
    NORMAL("normal"),

    /**
     * undetermined: The progressmeter is indeterminate.
     * 
     */
    @XmlEnumValue("undetermined")
    UNDETERMINED("undetermined");
    private final String value;

    ModetreecellAttributeType(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static ModetreecellAttributeType fromValue(String v) {
        for (ModetreecellAttributeType c: ModetreecellAttributeType.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
